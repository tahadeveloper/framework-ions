...
class Role extends Singleton
{
    ...
    {
        $tree = Control::hierarchy($language,[1], $level);

        $tree?->map(function ($row) use ($role_id, $language) {
            $is_exits = DB::connection(self::$connection_name)->table(self::$tables_names['roles_controls'])->where('role_id', $role_id)->where('control_id', $row->id)->exists();
            $row->checked = $is_exits;
            $actions = DB::connection(self::$connection_name)->table(self::$tables_names['actions'])->where('control_id', $row->id)->get();
            $actions->map(function ($action) use ($role_id, $language) {
                $action->language = DB::connection(self::$connection_name)
                    ->table(self::$tables_names['actions_languages'])
                    ->where('language', $language)
                    ->where('action_id', $action->id)->first();
                $is_exits = DB::connection(self::$connection_name)->table(self::$tables_names['roles_actions'])->where('role_id', $role_id)->where('action_id', $action->id)->exists();
                $action->checked = $is_exits;
                return $action;
            });
            $row->actions = $actions;
