...
{
    protected array $query = ['paging' => ['limit' => 100, 'offset' => 0], 'sort' => ['dir' => '', 'direction' => 'asc', 'column' => 'id'], 'filter_type' => 'or', 'filter' => []];
    ...
    public function toQuery(): array
    {
        ...
        return [
            ...
            'filter_type' => $this->query['filter_type'] ?? '',
    ...
    protected function filterQuery(): void
    {
        ...
        if (!empty($requestFilters['value'])) {
            $this->query['filter_type'] = 'or';
            $this->query['filter'] = [];
            foreach ($this->fields as $field) {
                if ($field['searchable'] === 'true') {
                    $this->query['filter'][$field['data'] . ':like'] = "%" . $requestFilters['value'] . "%";
                }
            }
        }
    }
